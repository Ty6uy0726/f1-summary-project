...
def main():
    ...
    while True:
        ...
        season_standings = get_season_standings(drivers_standings_df, driver_id, races_df)
        print(season_standings.to_string(index=False))
...
def get_season_standings(standings_df, driver_id, races_df):
    driver_standings = standings_df.loc[
        standings_df['driverId'] == driver_id, ['raceId', 'position', 'wins']
    ]

    driver_standings = driver_standings.merge(races_df[['raceId', 'year']], on='raceId')

    last_race_idx = driver_standings.groupby('year')['raceId'].idxmax()
    season_standings = driver_standings.loc[last_race_idx, ['year', 'position', 'wins']]

    return season_standings.sort_values('year').reset_index(drop=True)
